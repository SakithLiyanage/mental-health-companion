...
const EmotionTracker = () => {
  ...
  const loadEmotionEntries = React.useCallback(async () => {
    try {
      ...
      const response = await fetch('https://mental-health-companion-backend-eight.vercel.app//api/emotions?limit=100', {
        headers: {
  ...
  const handleLogEmotion = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        const response = await fetch('https://mental-health-companion-backend-eight.vercel.app//api/emotions', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      if (token) {
        const response = await fetch(`https://mental-health-companion-backend-eight.vercel.app//api/emotions/${editingEntry.id}`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const handleDelete = async (entryId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      if (token) {
        const response = await fetch(`https://mental-health-companion-backend-eight.vercel.app//api/emotions/${entryId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
